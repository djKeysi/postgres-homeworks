...
WHERE products.discontinued <> 1 AND products.units_in_stock <25 AND products.category_id IN(2,4) ORDER BY products.units_in_stock ASC
...
SELECT product_name FROM products WHERE product_id = ANY (SELECT product_id FROM order_details WHERE quantity = 10)